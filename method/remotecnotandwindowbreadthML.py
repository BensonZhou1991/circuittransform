...
This module is for search algorithm introducing remote CNOT and breadth first
search via ANN
"""
...
debug_mode = False
level_lookahead_default = [1, 0.9, 0.8, 0.7, 0.4] #default value [1, 0.8, 0.6, 0.4]
'''import ANN'''
...
def CalculateHeuristicCost(current_map, DG, executable_vertex, executed_vertex, shortest_length_G, shortest_path_G, SWAP_cost, max_shortest_length_G, level_lookahead, DiG):
    ...
    #cost_h_total_oldway = ct.HeuristicCostZhou1(current_map, DG, executed_vertex, executable_vertex, shortest_length_G, shortest_path_G, level_lookahead, DiG)
    cost_h_total = ct.HeuristicCostZhouML(ANN, current_map, DG, executed_vertex, executable_vertex, shortest_length_G, shortest_path_G, level_lookahead, DiG)
    #print('differential between distance is ', cost_h_total_oldway[1]-cost_h_total[1])
    cost_h1 = cost_h_total[1] * SWAP_cost
...
#@jit
def RemoteCNOTandWindowLookAheadML(q_phy, cir_phy, G, DG, initial_map, shortest_length_Gs, shortest_path_G, depth_lookahead, use_prune, draw=False, DiG=None, level_lookahead=None):
